<script lang="ts">
	...
	import IdeaDataTable from '$lib/components/idea-data-table.svelte';
	...
	async function fetchData() {
		if (channelId) {
			...
			const response = await fetch(`/idea?type=A&channelId=${channelId}`, {
				method: 'GET',
				headers: {
...
<SiteHeader page="Idea" callback={setChannelId} />
	...
	<div class="flex flex-1 flex-col">
		<div class="@container/main flex flex-1 flex-col gap-2">
			<div class="flex flex-col gap-4 py-4 md:gap-6 md:py-6">
				...
				<!-- {item} -->
				<!-- {/each} -->
				<IdeaDataTable {items} {isAdmin} {form} />
			</div>
		</div>
	</div>
	<!-- {/if} -->
