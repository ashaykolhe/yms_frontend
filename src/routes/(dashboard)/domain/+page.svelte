<script lang="ts">
	...
	import DomainDataTable from '$lib/components/domain-data-table.svelte';
	let channelId = $state('');
	...
	async function fetchData() {
		if (channelId) {
			...
			const response = await fetch(`/domain?type=A&channelId=${channelId}`, {
				method: 'GET',
				headers: {
...
<SiteHeader page="Domain" callback={setChannelId} />
	...
	<div class="flex flex-1 flex-col">
		<div class="@container/main flex flex-1 flex-col gap-2">
			<div class="flex flex-col gap-4 py-4 md:gap-6 md:py-6">
				...
				<!-- {item} -->
				<!-- {/each} -->
				<DomainDataTable {items} {isAdmin} {form} />
			</div>
		</div>
	</div>
	<!-- {/if} -->
